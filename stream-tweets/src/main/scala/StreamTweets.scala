...
//Data cleaning lambdas
   ...
   val tweets = tweetStripped.selectExpr("cast (tweet as string)")
         ...
         .withColumn("tweet", spaceRegex(col("tweet")))
         .withColumn("tweet", extract(col("tweet")))
...
//     .withColumn("wordle_id", row_number().over(Window.partitionBy(col("currentDate"))))//when (to_date(col("date")) === current_date(), )
...
/*  tweets.writeStream
  ...
  /*TODO: Add columns: Current Date/ timestamp, Batch Id/ auoto-generate wordle id based on date
